import { findOutStockOrderList } from '@/services/stock';
import { message } from 'antd';
import { ModelType, IQueryState, ConnectState } from '@/models/connect';
import { listDetailsBuild } from '@/services/_utils/helper';

export interface IOutstockList extends IQueryState {}

export default {
  namespace: 'outstockList',
  ...
  effects: {
    *loadList({ payload = {} }, { put, call, select }) {
      try {
        ...
        const resp = yield call(findOutStockOrderList, payload);
        const { data } = resp;
        if (data) {
          yield put({
